...
COREUTILS_VERSIONS = {
    "0.0.23": {
        ...
        # We casually break the naming convention here
        "linux_amd64_musl": {
            "filename": "coreutils-0.0.23-x86_64-unknown-linux-musl.tar.gz",
            "sha256": "sha256-lZvDYhmmvsETKeJCJBsC4wYblJ2CRvqAVCs62UC1TdM=",
        },
        "linux_arm64_musl": {
            "filename": "coreutils-0.0.23-i686-unknown-linux-musl.tar.gz",
            "sha256": "sha256-nEQJminqRsOUYqL1dCGeZ4rFIftwdX+PCOE49JQY+ts=",
        },
    },
}
...
def _coreutils_platform_repo_impl(rctx):
    ...
    postfix = ""
    if rctx.attr.musl:
        if rctx.attr.platform in ("linux_arm64", "linux_amd64"):
            postfix = "_musl"
        else:
            fail("Specified `musl` on platform that doesn't support it: " + rctx.attr.platform)

    filename = COREUTILS_VERSIONS[rctx.attr.version][platform + postfix]["filename"]
    ...
    rctx.download_and_extract(
        ...
        integrity = COREUTILS_VERSIONS[rctx.attr.version][platform + postfix]["sha256"],
    )
    build_content = """# @generated by @aspect_bazel_lib//lib/private:coreutils_toolchain.bzl
...
coreutils_platform_repo = repository_rule(
    ...
    attrs = {
        ...
        "musl": attr.bool(mandatory = True, default = False),
    },
)
