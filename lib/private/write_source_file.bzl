...
def _write_source_file_bat(ctx, paths):
    ...
    contents = ["""@rem @generated by @bazel_lib//:lib/private:write_source_file.bzl
@echo off
...
:done
""".format(
            in_path = to_rlocation_path(ctx, in_file),
    ...
    for update_script in additional_update_scripts:
        contents.append("call :rlocation {} script".format(to_rlocation_path(ctx, update_script)))
        contents.append("call %script%\n")
...
def _write_source_file_impl(ctx):
    ...
    if ctx.attr.in_file and out_file:
        ...
        paths.append((in_path, in_file, in_sub_path, out_path))
